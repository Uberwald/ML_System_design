...
- #### Гранулярность модели
  ...
  - Модель будет обновлятся каждую 1-2 недели
...
- Этап 1 - Отбор неабонентов и таргета в определенном интервале
...
- Этап 5 - Проведение ретро теста
- Этап 6 - Добавление новых источников и проведение тестов
Во время EDA было выявлено, что старый способ сбора таргета был некорректен, звонки дублировались всем, кроме даты.
Для каждого источника была проведена проверка на adversarial shift, которая на этот раз была пройдена.
Были добавлены новые источники.
Была получена таблица звонков за 6 месяцев, где данные за разные промежутки времени проходят проверку на различие.
- Этап 7 - Подготовка инференса модели
В качестве метрик были выбраны AUC и recall - первая показывает примерное качство модели, а вторая какой процент пользователей класса 1 был правильно определен.
В ходе работы было выявлено, что recall имеет более высокий приоритет, а также основные метрики проводились на топ 20% скоров.
ML-валидация производилась следюущим образом: в качестве валидационной выборки был взят промежуток с с июля по август, а в качестве тестовой за сентябрь, метрики улучшали
для валидации, добившись наилучшего результата проверяли на тесте.
Бейзлайн получался следующим образом: сбор таргета, сбор фичей, балансировка данных, обучение модели, подбор наилучших параметров.
Стратегии дальнейшего развития решения: коллеги из других команд посоветовали использовать отобранные фичи для моделей по кредитным картам абонентов (в данной работе используются НЕабоненоты), в теории, отбранные мною фичи должны улучшеить результат текущих моделей. Ещё один способ - собрать банковские данные именно на неабонентов - сейчас их фичи являются агрегированными фичами мобильного окружения неабонента (напомню, что по фичам окружения (людей, с которыми был совершен звонок) в теории можно предсказать вероятность оформления КК), также можно более аккуратно подойти к сбору окружения - исключить спам звонки и прочие.
Модель показала низкие метрики, но это связано с не очень хорошими данными и в целом со спецификой задачи. Ну и CatBoost зарекомендовал себя хорошо, особенно с балансировкой данных
менять модель пока не имеет никакого смысла.
Сложно что-то сказать про риски и способы их снижения, так как работа в большей мере исследовательская - мы исследуем возможности применения агрегированных данных окружения человека.
На текущий момент ожидается, что отборанные признаки для неабонентов (из 4000 осталось около 400) покажут хороший результат для абонентов.
Вообще тут есть несколько путей улучшения, я больше верю в последние два, нежели в применение фичей для моделей абонентов, но я исполнитель, а не заказчик, так что будем пробовать.

- Этап 8 - Подготовка документации и скриптов
...
- Описание формирования выборки для обучения, тестирования и валидации. Выбор репрезентативных данных для экспериментов, обучения и подготовки пилота (от бизнес-цели и репрезентативности данных с технической точки зрения) 
...
- Необходимый результат этапа. Например, необходимым результатом может быть не просто достижение каких-либо метрик качества, а включение в модели определенных факторов (флаг промо для прогноза выручки, др.) 
  - AUC не менее 0.6, recall не менее 0.6   
- Какие могут быть риски и что планируем с этим делать. Например, необходимый для модели фактор (флаг промо) окажется незначимым для большинства моделей. Или для 50% моделей будет недостаточно данных для оценки 
...
- Краткое описание предполагаемого дизайна и способа оценки пилота 
  - Планируется взять пользователей за период, не вошедший в обучение (октябрь, ноябрь) и посчитать для них скор, а потом проверить на нескольких топах (1 млн, 5 млн, 10 млн) конверсию, а также провести AB тест с предыдущим вариантом решения задачи
  
#### 3.2. Что считаем успешным пилотом  
  
- Конверсия чуть выше базового уровня (не могу привести цифры, но из-за специфики отрасли понятно, что в процентном соотношении разницу в конверсии почти не увидеть даже если мы сможем успешно подтолкнуть всех потенциальных клиентов к оформлению КК) [Требует доработки, пока не знаю точно]. Даже если АВ покажет результат хуже, это всё равно будет успех, так как здесь используются совершенно новые данные.
