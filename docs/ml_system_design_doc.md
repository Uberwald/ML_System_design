...
- Бизнес-цель `Product Owner`   
   Создать инструмент, который будет предсказывать наиболее склонных к оформлению кредитных карт клиентов банка, в малоизученном пользовательском сегменте. то позволит эффективно таргетировать маркетинговые кампании, а также поможет лучше изучить сегмент [клиента банка - не абонент мобильного оператора] в сравнении с уже хорошо изученным [клиента банка - абонент мобильного оператора]
- Почему станет лучше, чем сейчас, от использования ML `Product Owner` & `Data Scientist`   
Текущая ситуация:

    * Прогнозы делаются на основе только банковских источниках данных
    * Нет детализированной персонализации на уровне неабонентов, связанной с их окружением.

Улучшение с ML:

    Использование данных об окружении неабонента (агрегированные признаки абонентов, с которыми он общался) позволит строить более точные прогнозы его поведения.
    Автоматизация анализа большого объема данных, что ускоряет процесс и снижает трудозатраты.
    Повышение качества рекомендаций за счет использования исторических данных и сложных зависимостей, выявляемых моделью.
