...
- **Описание всех общих предпосылок решения, используемых в системе – с обоснованием от запроса бизнеса: какие блоки данных используем, горизонт прогноза, гранулярность модели, и др.** `Data Scientist`
  - Запрос бизнеса: Банк хочет определить, какие клиенты, не являющиеся абонентами МТС, с высокой вероятностью оформят кредитную карту. Это должно повысить эффективность маркетинговых кампаний.
  - Решение: Для построения модели используем данные об их взаимодействии с абонентами МТС (окружением) и банковские данные клиента. 
